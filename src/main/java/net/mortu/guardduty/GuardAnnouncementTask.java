...
public class GuardAnnouncementTask implements Runnable {
	...
	public GuardAnnouncementTask(GuardDuty plugin) {
		...
		Long interval = plugin.getConfig().getLong("schedules.announcements", 600L) * 20L;
	...
	@Override
	public void run() {
		if (plugin.getOnDutyGuardsCount() > plugin.getConfig().getInt("thresholds.announcements", 0)) {
			String message = plugin.getMessage(plugin.isKosActive() ? "announcements.kos-active" : "announcements.kos-inactive");
